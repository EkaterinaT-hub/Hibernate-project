package org.example.CountryCache;
...
import org.example.CountryCache.dao.CityDAO;
import org.example.CountryCache.dao.CountryDAO;
import org.example.CountryCache.domain.City;
import org.example.CountryCache.domain.Country;
import org.example.CountryCache.domain.CountryLanguage;
import org.example.CountryCache.redis.CityCountry;
import org.example.CountryCache.redis.Language;
...
public class Main {
    ...
    public void benchmarkBatch(List<Integer> ids, int warmupRuns, int measuredRuns) {
        ...
        try (StatefulRedisConnection<String, String> redisConn = redisClient.connect()) {
            ...
            System.out.printf("Redis (MGET + Jackson): %.3f ms avg (%d runs)%n",
                    redisTotal / 1_000_000.0 / measuredRuns, measuredRuns);

            System.out.printf("MySQL (IN + join fetch): %.3f ms avg (%d runs)%n",
